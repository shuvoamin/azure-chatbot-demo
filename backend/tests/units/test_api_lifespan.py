...
async def test_background_cleanup_task_cancelled():
    ...
    with patch("main.asyncio.to_thread", side_effect=asyncio.CancelledError()):
...
async def test_background_cleanup_task_exception():
    ...
    with patch("main.asyncio.to_thread", side_effect=side_effects):
        with patch("main.app_state.logger") as mock_logger:
